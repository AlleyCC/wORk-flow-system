...
const patientsServices = {
  ...
  getPreOp: (req, cb) => {
    return Patient.aggregate([
      { $match: { $and: [{ roomId: { $ne: null } }] } }, // 找出有房間id的資料($and must be an array)
      { $lookup: { from: 'rooms', localField: 'roomId', foreignField: '_id', as: 'room_data' } }, // 查詢關聯資料Room
      { $match: { 'room_data.statusCode': { $eq: 2 } } }, // 找出room.statusCode === 2
      {
        $lookup: { // 找出doctor的關聯資料
          as: 'doctor_data',
          from: 'doctors',
  ...
  getOp: (req, cb) => {
    return Patient.aggregate([
      { $match: { $and: [{ roomId: { $ne: null } }] } }, // 找出有房間id的資料($and must be an array)
      { $lookup: { from: 'rooms', localField: 'roomId', foreignField: '_id', as: 'room_data' } }, // 查詢關聯資料Room
      { $match: { 'room_data.statusCode': { $eq: 3 } } }, // 找出room.statusCode === 3
      {
        $lookup: { // 找出doctor的關聯資料
          as: 'doctor_data',
          from: 'doctors',
  ...
  getPostOp: (req, cb) => {
    return Patient.aggregate([
      { $match: { $and: [{ roomId: { $ne: null } }] } }, // 找出有房間id的資料($and must be an array)
      { $lookup: { from: 'rooms', localField: 'roomId', foreignField: '_id', as: 'room_data' } }, // 查詢關聯資料Room
      { $match: { 'room_data.statusCode': { $eq: 4 } } }, // 找出room.statusCode === 4
      {
        $lookup: { // 找出doctor的關聯資料
          as: 'doctor_data',
          from: 'doctors',
